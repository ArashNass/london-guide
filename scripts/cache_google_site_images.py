...
PHOTO_LOADER = r'''(() => {
  ...
  function itemFor(image) {
    ...
    if (path.includes("/views")) category = "views";
    else if (path.includes("/shopping")) category = "shop";
    else if (path.includes("/breakfast")) category = "breakfast";
    else if (path.includes("/restaurants")) category = "eat";
    else if (path.includes("/pubs")) category = "pubs";
    else if (path.includes("/apps")) category = "apps";
    return { id: `section-${alt || category || "london"}`, name: image.alt || "London", category, area: "London" };
  }
  ...
  function scan(root = document) {
    root.querySelectorAll?.(".card-media img, .modal-media img, .category-card img, .category-tile img, main img").forEach(prepare);
  }
...
.card-media img,.modal-media img,.category-card img,.category-tile img,main img{opacity:0;transition:opacity .28s ease}
.card-media img.photo-loaded,.modal-media img.photo-loaded,.category-card img.photo-loaded,.category-tile img.photo-loaded,main img.photo-loaded{opacity:1}
'''
...
def main() -> None:
    ...
    if css.exists():
        ...
        css.write_text(text.rstrip() + "\n/* real-photo-loader */" + STYLE_PATCH, encoding="utf-8")
